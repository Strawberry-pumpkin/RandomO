from flask import Blueprint, render_template, session, request, redirect, url_for, Flask, flash
    ...
    if request.method == 'POST':
        ...
        if contact_email != None and description != None:
            with open('/home/RandomO/mysite/static/contact.txt', 'r+')as cnt:
                ...
                cnt.seek(0)
                cnt.writelines('\n'.join(contact))
    return render_template('home.html', log_status=log_status, player_data=player_data)
...
def random_tasks():
    ...
    with open("/home/RandomO/mysite/static/tasks.txt", 'r') as ts:
        ...
        task1.append((tasks[r1], tasks[r1 + 1], tasks[r1 + 2], tasks[r1 + 3], tasks[r1+4]))
        task2.append((tasks[r2], tasks[r2 + 1], tasks[r2 + 2], tasks[r2 + 3],tasks[r2+4]))
        task3.append((tasks[r3], tasks[r3 + 1], tasks[r3 + 2], tasks[r3 + 3],tasks[r3+4]))
    ...
    return render_template('random.html', t1=task1, t2=task2, t3=task3, user=user, player_data=player_data, log_status=log_status)
...
def leaderboard():
    ...
    return render_template('leaderboard.html', data_table=data_table, player_data=player_data, log_status=log_status)
...
def unbelievable():
    ...
    return render_template ('unbelievable.html', player_data=player_data, log_status=log_status)
...
def emulator():
    ...
    return render_template('emulator.html', player_data=player_data, log_status=log_status)
